        ...
        for line in data.readlines():
            ...
            if not ( float(minRTT) > threshold and threshold > 0):
                ...
                if(self._setDisc.get(float(minRTT)) is None):
                    ...
                    #######  JOAN
                    tmp_disc=Disc(hostname,float(latitude),float(longitude),float(minRTT),int(TTL))
                    tmp_disc.setTime(time.time())
                    self._setDisc[float(minRTT)]=[tmp_disc]


                else:
                    tmp_disc=Disc(hostname,float(latitude),float(longitude),float(minRTT),int(TTL))
                    tmp_disc.setTime(time.time())
                    self._setDisc[float(minRTT)].append(tmp_disc)
        data.close()
